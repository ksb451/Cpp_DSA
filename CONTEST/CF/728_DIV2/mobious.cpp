...
void solve()
{
	ll n,M;
    cin>>n>>M;
    vector<vector<multiset<ll>>>date(13,vector<multiset<ll>>(31));
    vector<ll>power(n);
    vector<ll>month(n);
    vector<ll>day(n);
    ...
    for(int i=0;i<n;i++)
    {
    	date[month[i]][day[i]].insert(power[i]);
    }
    for(int i=0;i<n;i++)
    {
    	date[month[i]][day[i]].erase(date[month[i]][day[i]].find(power[i]));
    	for(int j=1;j<13;j++)
    	{
    		for(int k=1;k<31;k++)
    		{
    			if(date[j][k].empty())
    			{
    				...
    				ll curr = *(date[j][k].begin());
    				ll ans = power[i]*(abs(month[i]-j))+curr*(abs(day[i]-k));
    				if(ans<M)
    				{
    					cout<<1<<endl;
    					return;
