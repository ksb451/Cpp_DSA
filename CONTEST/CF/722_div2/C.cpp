...
vll adj[MAXN];
vector<vll>arr;
vector<vll> dfs(ll curr, ll par)
{
	vector<vll>ans(2,vll(2,0));
	ans[0][0] = arr[curr][0];
	ans[1][0] = arr[curr][1];
	...
	for(auto v:adj[curr])
	{
		if(v!=par)
		{
			vector<vll>temp = dfs(v,curr);
			ans[0][1]+= max((abs(ans[0][0]- temp[0][0])+temp[0][1]), (abs(ans[0][0]- temp[1][0])+temp[1][1]));
			ans[1][1]+= max((abs(ans[1][0]- temp[0][0])+temp[0][1]), (abs(ans[1][0]- temp[1][0])+temp[1][1]));
		}
	}
	return ans;
}
