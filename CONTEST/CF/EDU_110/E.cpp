...
void solve()
{
	...
	ll n;
    cin>>n>>cost[0].first>>cost[0].second;
    for(int curr=1;curr<=n;curr++)
    {
    	...
    	if(typ==1)
    	{
    		cin>>blift[curr][0]>>cost[curr].first>>cost[curr].second;
    		for(int i=1;i<K;i++)
    		{
    			if(blift[curr][i-1]!=-1)
    				blift[curr][i] = blift[blift[curr][i-1]][i-1];
    		}
    	}
    	else{
    		ll ans_tons=0;
    		...
    		while(W)
    		{
    			...
    			if(cost[req].first==0)
    			{
    				...
    				ll temp = min(cost[req].first,W);
    				W-=temp;
    				cost[req].first -= temp;
    				ans_tons+=temp;
    				ans_amt+=(temp * cost[req].second);
    			}
    		}
    		cout<<ans_tons<<" "<<ans_amt<<endl;
    		cout.flush();
    	}
    }
    return;
