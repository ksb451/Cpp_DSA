...
struct treenode{
	ll cnt;
	treenode* next[2];
	treenode(){cnt=0;next[0]=next[1]=nullptr;}
};

treenode *root;

void add_to_trie(ll num)
{
	treenode* cur;
	cur = root;
	for(ll j=31;j>=0;j--)
...
void helper(ll x)
{
	...
	for(ll j =31;j>=0;j--)
	{
		...
		else if(curr->next[ind]->cnt < (1LL<<j)){
			curr = curr->next[ind];
		}
		else{
			ans+= (1LL<<j);
			if(curr->next[ind^1] == nullptr)
			{
				cout<<ans<<endl;
				return;
			}
			else{
				curr = curr->next[ind^1];
			}
		}
	}
	cout<<ans<<endl;
