...
vll dfs(ll curr, ll par , ll mn_val)
{
	vll cnt = {0,0};
	if(ini[curr] != fin[curr])
	{
		cnt[ini[curr]]++;
	}
	...
	for(auto v:adj[curr])
	{
		if(v!=par)
		{
			vll x = dfs(v,curr, min(mn_val,cost[curr]));
			cnt[0]+=x[0];
			cnt[1]+=x[1];
		}
	}
	//__print(curr, cnt[0], cnt[1]);nl();
	if(cost[curr] <= mn_val)
	{
		ll temp_mn = min(cnt[0], cnt[1]);
		ans+= cost[curr]*(temp_mn*2);
		cnt[0]-=temp_mn;
		cnt[1]-=temp_mn;
	}
	return cnt;
